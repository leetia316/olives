...
public class AuthorityController extends JavaEEFrameworkBaseController<Authority> implements Constant {
	...
	@RequestMapping("/makeHomePage")
	public void makeHomePage(HttpServletRequest request ,HttpServletResponse response) throws Exception{
		List<Authority> authoritiesList = authorityService.queryHomePageList(HOME_PAGE);
		boolean flag = authorityService.buildHomePageMenuXml(authoritiesList);
		if (flag) {
			writeJSON(response, "{success:true}");
		} else {
			writeJSON(response, "{success:false}");
