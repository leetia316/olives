var AW_TEMPLATE = {
				...
				'<div class="modal-content">'+
					...
					'<form id="favorite_form" action="' + G_BASE_URL + '/favorite/ajax/update_favorite_tag/" method="post" onsubmit="return false;">'+
						...
						'<div class="aw-favorite-tag-list">'+
							...
							'</div>'+
						'</div>'+
						'<div class="aw-favorite-tag-add hide">'+
							'<div class="modal-body">'+
							...
							'<div class="modal-footer">'+
								...
								'<button href="javascript:;" class="btn btn-normal btn-primary" onclick="AWS.User.add_favorite_tag()">' + _t('确认创建') + '</button>'+
							'</div>'+
						'</div>'+
					'</form>'+
				'</div>'+
					...
					'<div class="aw-comment-box-main">'+
						...
						'<div class="aw-comment-box-btn">'+
							'<span class="pull-right">'+
								...
								'<a href="javascript:;" class="btn btn-mini btn-primary btn-reply" onclick="AWS.User.save_comment($(this));">' + _t('评论') + '</a>'+
							'</span>'+
						'</div>'+
					'</div>'+
				'</form>'+
	...
	// Modify by wecenter
	'gonodataReference' :
		'<div class="modal fade alert-box gonodataReference-box">'+
			'<div class="modal-dialog">'+
				'<div class="modal-content">'+
					'<div class="modal-header">'+
						'<a type="button" class="close icon icon-delete" data-dismiss="modal" aria-hidden="true"></a>'+
						'<h3 class="modal-title text-center" id="myModalLabel">{{title}}' + _t('的参考文档') + '</h3>'+
					'</div>'+
					'<div class="modal-body clearfix">'+
						'<div class="mod-body">'+
							'<ul></ul>'+
	...
	// Modify by wecenter
	'refound' :
		'<div class="modal fade alert-box request-box">'+
			'<div class="modal-dialog">'+
				'<div class="modal-content">'+
					'<div class="modal-header">'+
						'<a type="button" class="close icon icon-delete" data-dismiss="modal" aria-hidden="true"></a>'+
					...
					'<div class="modal-body clearfix">'+
						'<div class="mod-body">'+
							'<div class="alert alert-danger hide error_message"><i class="icon icon-delete"></i> <em></em></div>'+
							'<p class="hide success" style="padding:40px 95px;text-align:center;">WeGene 后台已接受您的申请，24小时内将会有工作人员与你联系。请您留意。谢谢。</p>'+
							'<form id="request_form" action="'+ G_BASE_URL +'/account/ajax/add_sample_fail_request/" method="post">'+
								'<input type="text" class="hide" name="process_mode" value="{{process_mode}}"/>'+
								'<dl>'+
									'<dt>套件码 :</dt>'+
									'<dd><input class="form-control" type="text" name="barcode" value="{{barcode}}" readonly/></dd>'+
								'</dl>'+
								'<dl>'+
									'<dt>订单号 :</dt>'+
									'<dd><input class="form-control" type="text" name="order_id"  /><p class="text-color-999">选填, 退款需要尽量提供订单号</p></dd>'+
								'</dl>'+
								'<dl>'+
									'<dt>留言 :</dt>'+
									'<dd><textarea class="form-control" name="message" rows="4"></textarea><p class="text-color-999">请在留言中提供联系方式, 若要重发样本请在留言中提供邮寄地址</p></dd>'+
								'</dl>'+
								'<dl>'+
									'<dt></dt>'+
									'<dd><a class="btn btn-primary btn-normal" onclick="AWS.ajax_post($(\'#request_form\'), \'\', \'refound_processer\');">提交</a></dd>'+
								'</dl>'+
							'</form>'+
						'</div>'+
					'</div>'+
	...
	'customerClient': 
		'<div class="modal fade alert-box customerClient-box">'+
			'<div class="modal-dialog">'+
				'<div class="modal-content">'+
					'<div class="modal-header">'+
						'<a type="button" class="close icon icon-delete" data-dismiss="modal" aria-hidden="true"></a>'+
					...
					'</div>'+
					'<div class="modal-footer">'+
						'<a class="btn btn-primary btn-large disabled pull-right" onclick="AWS.ajax_request(\'' + G_BASE_URL + '/account/ajax/join_research_project/\', \'next=' + G_RESEARCH + '\')">我已阅读</a>'+
					'</div>'+
				'</div>'+
			'</div>'+
		'</div>',
	...
	'dataDownload': 
		'<div class="modal fade alert-box dataDownload-box">'+
			'<div class="modal-dialog">'+
				'<div class="modal-content">'+
					'<form id="download_form" action="report/download/" method="post">'+
						...
						'<div class="modal-body clearfix">'+
							'<div class="mod-body">'+
								'<input type="password" name="password" class="form-control" placeholder="' + _t('请输入密码') + '" />'+
							'</div>'+
						'</div>'+
						'<div class="modal-footer">'+
							'<button class="btn btn-normal btn-primary" type="submit" onclick="$(\'.modal\').modal(\'hide\')">' + _t('下载') + '</a>'+
						'</div>'+
					'</form>'+
				'</div>'+
			'</div>'+
