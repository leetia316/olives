...
$(document).ready(function ()
{
    ...
    setTimeout(function ()
    {
        if ($('body').children().eq($('body').children().length - 1).attr('id'))
        {
            if ($('body').children().eq($('body').children().length - 1).attr('id').match('xunlei'))
            {
                $('body').children().eq($('body').children().length - 1).detach();
            }
        }
    }, 500);
    ...
    $('.aw-top-nav .navbar-toggle').click(function ()
    {
        if ($('body').hasClass('animate-show'))
        {
            ...
            $('.aw-top-nav .side-nav').removeClass('animate-show').addClass('animate-hide');
        }
        else
        {
            $('body').attr('style', 'width:' + $(window).width() + 'px;').removeClass('animate-hide').addClass('animate-show');

            $('.aw-top-nav .side-nav').removeClass('animate-hide').addClass('animate-show');
        }
    });
    ...
    // Modify by wecenter
    /*icon tooltips提示*/
    $(document).on('mouseover', '.icon-good, .icon-bad, .follow, .voter, .aw-icon-thank-tips, .invite-list-user, .tooltips', function ()
    {
        $(this).tooltip('show');
    });
