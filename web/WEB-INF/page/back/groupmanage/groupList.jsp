				...
				<div class="modal-body" style="max-height: 500px;overflow-y: scroll;">
						...
						<table width="100%">
							<tr style="height:40px; list-style-type: none;">
								<td width="300" align="right">
									<label style="font-family: 'Courier New'">讨论组名称:</label>
								</td>
								<td width="500" align="left">
									<div class="col-sm-6 col-xs-12">
										<div class="input-icon block col-xs-12 no-padding">
											<i class="ace-icon fa fa-comments"></i>
											<input type="text" class="col-xs-12" name="groupName" id="groupName"  placeholder="请输入讨论组名称" />
					...
					<div class="blue clearfix">
						<table width="100%">
							<tr style="height:40px; list-style-type: none;">
								<td width="300" align="right">
									<label style="font-family: 'Courier New'">讨论组类型:</label>
								</td>
								<td width="500" align="left">
									<div class="col-sm-6 col-xs-12">
										<div class="input-icon block col-xs-12 no-padding" id="groupTypeInfo">
					...
					<div class="blue clearfix">
						<table width="100%">
							<tr style="height:40px; list-style-type: none;">
								<td width="300" align="right">
									<label style="font-family: 'Courier New'">讨论组排序:</label>
								</td>
								<td width="500" align="left">
									<div class="col-sm-6 col-xs-12">
										<div class="input-icon block col-xs-12 no-padding">
											<i class="ace-icon fa fa-flask"></i>
											<input onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')" type="text" class="col-xs-12" name="groupSort"  id="groupSort"  placeholder="请输入排序值必须为数字" />
					...
					<div class="blue clearfix">
						<table width="100%">
							<tr style="height:40px; list-style-type: none;">
								<td width="300" align="right">
									<label style="font-family: 'Courier New'">讨论组图片:</label>
								</td>
								<td width="500" align="left">
									<div class="col-sm-6">
										...
										<input type="text" readonly style="display: none" maxlength="20" class="col-xs-12" id="groupImg" name="groupImg" />
									</div>
					...
					<div class="blue clearfix">
						<table width="100%">
							<tr style="height:60px; list-style-type: none;">
								<td width="300" align="right">
									<label style="font-family: 'Courier New'">讨论组备注:</label>
								</td>
								<td width="500" align="left">
									<div class="col-sm-6 col-xs-12">
										<div class="input-icon block col-xs-12 no-padding">
											<textarea class="form-control limited" id="groupMemo" name="groupMemo" maxlength="100" placeholder=""></textarea>
...
<script type="text/javascript">
	...
	$(".page-content-area").ace_ajax("loadScripts", scripts, function() {
		...
		jQuery(function($) {
			...
			jQuery(grid_selector).jqGrid({
				subGrid : false,
				url : "${contextPath}/sys/group/getGroup",
				datatype : "json",
				height : 450,
				colNames : ['操作', 'ID', '讨论组名称', '讨论组类型', '讨论个数','关注个数','排序值','LOGO','备注'],
				colModel : [ {
					name : '',
					index : '',
					width : 60,
					...
					edittype : "select",
					editoptions : {
						dataUrl : "${contextPath}/sys/dict/getDictSelectList"
					},
					search:false,
				}, {
					name : 'groupSize',
				...
				editurl : "${contextPath}/sys/group/operateGroup",
				//caption : "用户管理列表",
				//autowidth : true,
				/**
				 grouping : true,
			...
			$("#addInformationButton").bind("click", function() {
				...
				jQuery.ajax({
					type: 'get',
					datatype : "json",
					url: "${contextPath}/sys/dict/getDictSelectList" ,
					success: function(data){
						$("#groupType").empty();
						$("#groupType").append("<option value=''></option>");
						for(var i=0;i<eval(data).length;i++){
							$("#groupType").append("<option value='"+eval(data)[i].dictKey+"'>"+eval(data)[i].dictValue+"</option>");
						}
						jQuery("#defaultIconUpload").show();
						jQuery("#btnInfo").hide();
						jQuery("#groupImg").hide();
						jQuery("#imgInfo").html("");
			...
			$("#editInformationButton").bind("click", function() {
				...
				if(null == selectedId){
					...
					$("#groupName").val(jQuery(grid_selector).jqGrid("getRowData",jQuery(grid_selector).jqGrid("getGridParam", "selrow")).groupName);
					$("#groupSort").val(jQuery(grid_selector).jqGrid("getRowData",jQuery(grid_selector).jqGrid("getGridParam", "selrow")).groupSort);
					$("#groupMemo").val(jQuery(grid_selector).jqGrid("getRowData",jQuery(grid_selector).jqGrid("getGridParam", "selrow")).groupMemo);
					...
					jQuery.ajax({
						...
						success: function(data){
							$("#groupType").empty();
							for(var i=0;i<eval(data).length;i++){
								if(eval(data)[i].dictValue == typeInfo){
									$("#groupType").append("<option value='"+eval(data)[i].dictKey+"' selected>"+eval(data)[i].dictValue+"</option>");
								}else{
									$("#groupType").append("<option value='"+eval(data)[i].dictKey+"' >"+eval(data)[i].dictValue+"</option>");
								}
							}
					...
					var imageInfo = jQuery(grid_selector).jqGrid("getRowData",jQuery(grid_selector).jqGrid("getGridParam", "selrow")).groupImg;
					jQuery("#groupImg").show();
					jQuery("#groupImg").val(imageInfo);
			...
			jQuery("#submitButton").click(function(){
				...
				var groupType = jQuery("#groupType").val();
				if(groupName ==''){
					message+='提示：讨论组名称不能为空';
				}
				if(groupType ==''){
					message+=' &nbsp;&nbsp;&nbsp;提示：讨论组类型不能为空'
				}
				if(message != ''){
					jQuery('#tipInfo').show();
				...
				jQuery.ajax({
					dataType : "json",
					url : "${contextPath}/sys/group/operateGroup?oper=add",
					type : "post",
					data : {
						id : $("#id").val(),
						groupName : $("#groupName").val(),
						groupType : $("#groupType").val(),
						groupSort : $("#groupSort").val(),
						groupImg : $("#groupImg").val(),
	...
	function ajaxUpload(id,imgId,hiddId){
		...
		(
				{
					url: "${contextPath}/sys/fileUpload/app?dir=GROUP", //你处理上传文件的服务端
					secureuri:false,
					fileElementId:id,
					dataType: 'json',
					success: function (data, status)
					{
						jQuery("#groupImg").val("${contextPath}/static/upload/"+data.file_path);
